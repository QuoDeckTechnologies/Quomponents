...
export default function RewardCard(props) {
  ...
  const [stockStyle, setStockStyle] = useState(
    "qui-reward-card-cost-stock-container-row"
  );
  ...
  useEffect(() => {
    ...
    cost?.length > 7 || left?.length > 7 || total?.length > 7
      ? setStockStyle("qui-reward-card-cost-stock-container-column")
      : setStockStyle("qui-reward-card-cost-stock-container-row");
  ...
  const getRewardCard = (asEmphasis) => {
    ...
    if (asEmphasis === "soldout") {
      return (
        <div>
          {contentName && (
            <div className="qui-reward-card-name">{contentName}</div>
          )}
          {rewardImage && (
            <div
            ...
            >
              <div className="qui-reward-card-image-caption">{soldoutText}</div>
            </div>
          )}
