...
export default function RewardBadge(props) {
  ...
  return (
    <div className={`qui ${quommonClasses.parentClasses}`}>
      <div
        className={`qui-reward-badge-container ${
          props?.withLabel?.format === "popover" ? "popover" : ""
        }  qui-btn ${quommonClasses.childClasses}`}
        style={{
      ...
      >
        <div className="qui-badge-label qui-label">
          <h4>{getLabel(labelContent, "label")}</h4>
        </div>
        {props?.image && (
          <img
        ...
        <div className="qui-badge-caption qui-caption">
          <h4>{getLabel(labelContent, "caption")}</h4>
        </div>
      </div>
    </div>
  );
