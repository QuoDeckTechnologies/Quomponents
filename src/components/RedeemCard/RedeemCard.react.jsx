...
export default function RedeemCard(props) {
    ...
    useEffect(() => {
        ...
        props.description?.length > 113 ? setShowMoreBtn(true) : setShowMoreBtn(false);
        cost?.length > 4 || left?.length > 4 || total?.length > 4 ? setStockStyle("qui-redeem-card-cost-stock-container-column") : setStockStyle("qui-redeem-card-cost-stock-container-row");
    }, [showMoreBtn, props.description, props.cost, props.stock?.left, props.stock?.total]);
    //-------------------------------------------------------------------
